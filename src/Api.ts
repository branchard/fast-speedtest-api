        ...
        .get(options, (response) => {
          if (response.headers["content-type"].includes("json")) {
            ...
            response.on("end", () => {
              ...
              (response as any).data = parsedData;
              resolve({
                response,
                request,
              });
  ...
  async getTargets() {
    try {
      ...
      while (targets.length < this.urlCount) {
        ...
        if (this.proxy) (options as any).agent = this.proxy;
        /* eslint-disable no-await-in-loop */
        const { response } = await (this.get(options) as any);
  ...
  async getSpeed() {
    ...
    return new Promise((resolve) => {
      ...
      const refreshIntervalId = setInterval(() => {
        ...
        if (this.verbose) {
          console.log(
            `Current speed: ${this.unit(Api.average(recents))} ${
              this.unit.name
            }`
          );
        }
      ...
      timer.addCallback(() => {
        ...
        resolve(this.unit(Api.average(recents)));
      });
